...
class DiverseMiniBatchLearner(ActiveLearnerBase):
    ...
    """
    def __init__(self, col_names: List[str], scorer: BaseEstimator, beta: int = 5, batch_size: int = 5, min_nr_batch: int = 2,
            ...
            if user_input == 'p':
                if y_new:
                    ...
                    user_input_prev = self.get_active_learning_input(query_inst.loc[prev_index]
                                                                      , choices=['y', 'n', 's'],
                                                                     message = "Correcting previous label, please choose (y)es, (n)o, or (s)kip")[0]
